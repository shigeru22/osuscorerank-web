	...
	useEffect(() => {
		async function getScores() {
			...
			if(!_.isUndefined(scores.data)) {
				...
				addLogData("Info", "Fetch global ranking success.");
			}
			else {
				addLogData("Error", `Fetch global ranking failed: ${ scores.message }`);
			}
			...
			<div className="2xl:flex 2xl:justify-between 2xl:gap-x-6 gap-y-6 space-y-6 2xl:space-y-0">
				<div className="flex flex-col gap-y-6">
					...
					<div className="flex 2xl:flex-col items-start gap-x-4 gap-y-4 px-8 md:px-0 overflow-x-auto">
						...
						<StatsCard title="Total Inactives" data={ totalInactives.toString() } subtitle="+ 5 since last month" />
					</div>
				</div>
